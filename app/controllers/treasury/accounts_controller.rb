class Treasury::AccountsController < ApplicationController
  ...
  def show
    ...
    if params[:download] == 'true'
      ...
      # send_data @treasury_account.csv_ledger(params[:date_from], params[:date_to]),
      #           filename: "#{@treasury_account.name.downcase}_ledger_#{params[:date_from]}_to_#{params[:date_to]}",
      #           type: "text/csv",
      #           disposition: 'attachment'

      flash.now[:notice] = "CSV will be generated and will be ready once done"
    else
