...
class Claims::ClaimPaymentsController < ApplicationController
  before_action :set_claim_payment, only: %i[ show edit update destroy ]

  # GET /claim_payments
  def index
    @claim_payments = Claims::ClaimPayment.all
  end
  ...
  def new
    @claim_payment = Claims::ClaimPayment.new
  end
  ...
  # POST /claim_payments
  def create
    @claim_payment = Claims::ClaimPayment.new(claim_payment_params)
  ...
  private
    ...
    def set_claim_payment
      @claim_payment = Claims::ClaimPayment.find(params[:id])
    end
