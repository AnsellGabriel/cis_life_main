class Claims::ClaimRemarksController < ApplicationController
  ...
  def index
    if current_user.userable == "CoopUser"
      ...
      @claim_remarks = Claims::ClaimRemark.joins(:process_claim).where(read: 0, process_claim: { cooperative_id: @cooperative.id }).where.not(user: current_user)
    else
      @claim_remarks = Claims::ClaimRemark.joins(:process_claim).where(read: 0).where.not(user: current_user)
    end
  ...
  def new
    ...
    # @claim_remarks = @process_claim.claim_remark.where(coop: 1)
    # raise "error"
  ...
  def update
    respond_to do |format|
      if @claim_remark.update(claim_remark_params)
        format.html { redirect_back fallback_location: claim_process_claims_process_claim_path(@process_claim), notice: "Claim remark was successfully updated." }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to claim_process_claims_process_claim_path(@process_claim), notice: "Claim remark was successfully destroyed." }
  ...
  def set_claim_remark
    @claim_remark = Claims::ClaimRemark.find(params[:id])
    @process_claim = @claim_remark.process_claim
  end

  # Only allow a list of trusted parameters through.
  def claim_remark_params
    params.require(:claims_claim_remark).permit(:process_claim_id, :user_id, :status, :remark, :coop, :read)
  end
end
