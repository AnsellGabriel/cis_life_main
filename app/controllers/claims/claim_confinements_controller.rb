...
class Claims::ClaimConfinementsController < ApplicationController
  ...
  def set_claim_confinement
    @claim_confinement = Claims::ClaimConfinement.find(params[:id])
    @process_claim = @claim_confinement.process_claim
    @benefit = Claims::Benefit.find_by(name: "Hospital Income Benefit")
  end

  # Only allow a list of trusted parameters through.
  def claim_confinement_params
    params.require(:claims_claim_confinement).permit(:process_claim_id, :date_admit, :date_discharge, :terms, :amount)
  end
end
