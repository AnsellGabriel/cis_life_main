class Claims::ClaimRequestForPaymentsController < ApplicationController
  before_action :set_claim_request_for_payment, only: %i[ show edit update destroy ]

  # GET /claim_request_for_payments
  def index
    @claim_request_for_payments = Claims::ClaimRequestForPayment.all
  end
  ...
  def new
    @claim_request_for_payment = Claims::ClaimRequestForPayment.new
  end
  ...
  def create
    @claim_request_for_payment = Claims::ClaimRequestForPayment.new(claim_request_for_payment_params)
  ...
  private
    ...
    def set_claim_request_for_payment
      @claim_request_for_payment = Claims::ClaimRequestForPayment.find(params[:id])
    end
