class Actuarial::Reserve < ApplicationRecord
  ...
  def update_totals
    self.update(
      total_unearned_prem: self.batches.sum(:unearned_prem),
      total_first_advance_prem: self.batches.sum(:first_adv_prem),
      total_second_advance_prem: self.batches.sum(:second_adv_prem),
      total_reserve: self.batches.sum(:reserve_amt),
      total_unearned_pr: self.batches.sum(:unearned_pr),
      total_first_advance_pr: self.batches.sum(:first_adv_pr),
      total_second_advance_pr: self.batches.sum(:second_adv_pr),
      total_reserve_ret: self.batches.sum(:reserve_ret_amt)
    )
  end
  ...
  def get_reserve_batches
    ...
    batches.each do |batch|
      r_batch = Actuarial::ReserveBatch.find_or_initialize_by(reserve: self, batch: batch)
      r_batch.term = batch.terms
      r_batch.rate = LoanInsurance::Rate.find_by(id: batch.loan_insurance_rate_id).monthly_rate
      r_batch.coverage_less_ri = batch.loan_amount > 550000 ? batch.loan_amount - 550000 : batch.loan_amount
